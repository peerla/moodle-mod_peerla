...
require_once realpath(__DIR__).'/AverageCalculationDataPoint2D.php';

/**
 * A data view tree node, that generates the linear average of all child nodes
 * data points.
 *
 * @author Christoph Bohr
 */
class ChildAverageDataViewNode extends BasicDataViewNode{
		...
		foreach($this->getChildNodes() as $node){
			foreach($node->getDataPoints() as $dataPoint){
				...
				if (!isset($avgDataPoints[$value])){
					$newPoint = new AverageCalculationDataPoint2D();
					if ($this->groupBy == 'x'){
